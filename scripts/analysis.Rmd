...
```{r, cache=TRUE, message=FALSE, fig.cap="Proprtion of replicates surviving until end"}
...
```{r, cache=TRUE, warning=FALSE, fig.cap="Final maximum fitness"}
...
```{r, cache=TRUE, warning=FALSE, fig.cap="Final average fitness"}
ggplot(data = survivals %>% 
                filter(gen==51) %>% 
...
```{r, cache=TRUE, fig.cap="Maximum fitness over time"}
ggplot(data = trace %>% 
...
```{r, cache=TRUE, fig.cap="Average fitness over time"}
ggplot(data = trace %>% 
         filter(mu==.01, k==10), 
       aes(x = gen, y = average)) +
  facet_wrap(~regime) +
...
```{r, cache=TRUE, fig.cap="Unique genotypes over time"}
ggplot(data = trace %>% 
         filter(mu==.01, k==10), 
       aes(x = gen, y = uniques)) +
...
```{r, cache=TRUE, fig.cap="Unique genotypes over time, normalized by population size"}
ggplot(data = trace %>% 
         filter(mu==.01, k==10), 
...
```{r, cache=TRUE, warning=FALSE, fig.cap="Correlation between reproductive and shock fitness in evolved genomes"}
...
```{r, cache=TRUE, warning=FALSE, fig.cap="Graph of the full reproductive fitness landscape with final population size represented by node size"}
...
```{r, cache=TRUE, warning=FALSE, fig.cap="Graph of the full shock fitness landscape with final population size represented by node size"}
ggraph(graph, layout = 'manual', x=max(ones)/2 * (pos)/(total+1), y=ones) +
